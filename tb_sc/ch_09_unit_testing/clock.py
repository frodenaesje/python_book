# file: Clock.py
# This file is a copy of sc_08_16_clock.py, so it can be imported
# by the test file in the simplest possible way.
...
class Clock:
    ...
    def inc_day(self):
        # Implementation is completed as an exercise.
        pass
...
def main():
    print("=== Initialization and __str__() ===")
    ...
    clock.minute = 45
    print(clock)                             # 2023-06-15 00:45:00
    clock.month  = 1
    clock.day    = 31
    # February: day 31 becomes 1.
    clock.month  = 2
    print(f"Day after month change: {clock.day}")   # 1

    print("\n=== Leap years ===")
    leap    = Clock(2020, 2, 29)
    print(leap)                              # 2020-02-29 00:00:00
    no_leap = Clock(2021, 2, 29)
    print(no_leap)
    # corrected to 2021-02-01

    print("\n=== inc_sec() and inc_min() - implemented ===")
    clock = Clock(2023, 1, 1, 0, 0, 58)
    clock.inc_sec()
    print(clock)                             # 2023-01-01 00:00:59
    clock.inc_sec()
    print(clock)                             # 2023-01-01 00:01:00

    print("\n=== inc_hour() -> inc_day() ===")
    print("=== inc_day() not implemented ===")
    clock = Clock(2023, 1, 15, 23, 0, 0)
    clock.inc_hour()
    print(clock)                             # 2023-01-15 00:00:00
    # inc_hour() calls inc_day() at midnight, but inc_day() has not
    # been implemented yet, so the day does not change as expected.
    print(f"Day (expected 16, actual): {clock.day}")
    # 15 - not implemented
